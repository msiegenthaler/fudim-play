...
object DatabaseCubeData {
  private sealed trait DCDBase[D] extends EditableCubeData[D] with AbstractCubeData[D] {
    ...
    def create: Unit = DB.withConnection { implicit c ⇒
    ...
    def drop: Unit = DB.withConnection { implicit c ⇒
  ...
  private case class DCDString(table: String, dims: Map[Dimension, String], slice: Point, filters: DimensionFilter = Map.empty) extends DCDBase[String] {
